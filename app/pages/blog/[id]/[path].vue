...
<template>
    <UContainer>
        ...
        <UPage>
            ...
            <UPageBody>
                <NuxtImg v-if="article?.image" :src="article?.image" :alt="article?.title || 'บทความ'"
                    class="w-full h-auto rounded-xl mt-4 mb-8"
                ...
                <div class="flex justify-end  gap-2">
                    <UButton icon="i-simple-icons-facebook" color="primary" variant="soft" size="lg"
                        :to="`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(`https://lmbaccount-law.com/blog/${id}/${path}`)}`"
                        target="_blank" aria-label="แชร์บทความนี้ บน Facebook" />
                    <UButton icon="i-simple-icons-line" color="success" variant="soft" size="lg"
                        :to="`https://social-plugins.line.me/lineit/share?url=${encodeURIComponent(`https://lmbaccount-law.com/blog/${id}/${path}`)}`"
                        target="_blank" aria-label="แชร์บทความนี้ บน LINE" />
                    <UButton icon="i-simple-icons-x" color="neutral" variant="soft" size="lg"
                        :to="`https://twitter.com/intent/tweet?url=${encodeURIComponent(`https://lmbaccount-law.com/blog/${id}/${path}`)}&text=${encodeURIComponent(article?.title || '')}`"
                        target="_blank" aria-label="แชร์บทความนี้ บน X" />
            ...
            <UPageCTA title="ต้องการคำปรึกษาเพิ่มเติม?"
                ...
                variant="soft" :links="[
                    ...
                    { label: 'โทรหาเรา 084-387-7167', icon: 'i-heroicons-phone', color: 'neutral', variant: 'outline', size: 'xl', to: 'tel:0843877167' }
                ]" />
            <template #right>
                <UPageAside>
                    <!-- Social Share -->
                    <div class="mb-8">
                        ...
                        <div class="flex gap-2">
                            ...
                            <UButton icon="i-heroicons-link" color="secondary" variant="soft" size="lg"
                                @click="copyLink" aria-label="คัดลอกลิงก์"/>
                        </div>
                    </div>
